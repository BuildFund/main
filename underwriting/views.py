...
def build_prompt(project: Project, report_type: str) -> list[Dict[str, str]]:
    ...
    user_prompt = (
        ...
        f"Funding type: {project.get_funding_type_display()}.\n"
        f"Property type: {project.get_property_type_display()}.\n"
        f"Development extent: {project.get_development_extent_display()}.\n"
        f"Tenure: {project.get_tenure_display()}.\n"
        f"Loan amount required: {project.loan_amount_required}.\n"
        f"Term required (months): {project.term_required_months}.\n"
        f"Repayment method: {project.get_repayment_method_display()}.\n"
        f"Gross development value: {project.gross_development_value}.\n"
        f"Current market value: {project.current_market_value}.\n"
        f"Please write a comprehensive underwriting report for a {report_type} perspective."
    )
    return [
